...
class Photo(object):
    """
    Object which represents a photo on the file system and, possibly, in the
    database
    """
    ...
    def _set_internal(self, name, value, bypass_checks=False, set_dirty=True):
        ...
        :param name: name of the attribute to set
        :param value: value for the attribute
        :param bypass_checks: whether or not to bypass the checks
        :param set_dirty: whether or not to set this object as dirty
        """
    ...
    @property
    def tags(self):
        """return the EXIF tags for the file"""
    ...
    def get_count_by_date(year=None, month=None, day=None, field="image_date"):
        ...
        :param year: year for the count of files
        :param month: month for the count of files
        :param day: month for the count of files
        :param field: which date field to compare
    ...
    def get_by_hash(hashes):
        ...
        :param hashes: a list of hashes or a single hash to fetch

        :returns dict: keyed by hash of lists of Photo objects
        """
        field = FieldArg("*")
    ...
    def get_by_date(timestamp=None, year=None, month=None, day=None, field="image_date", limit=None, offset=None):
        ...
        :param year: what year to fetch
        :param month: what month to fetch
        :param day: what day to fetch
        :param field: which type of date to compare to
        :param limit: how many photo objects to fetch
        :param offset: how many photos to skip
    ...
    def get_marked(offset=None, limit=None):
        ...
        :param offset: how many photos to skip
        :param limit: how many photos to return
    ...
    def get_all_dates(year=None, month=None):
        ...
        :param year: which year to fetch
        :param month: which month to fetch
    ...
    def get_from_data(**kwargs):
        ...
        :param kwargs:
            ...
            image_date - date the photo was taken
            hash - file has for the photo
            marked - whether the photo is marked for deletion

        :returns Photo: A Photo object built from the provided data
        """
    ...
    def get_by_path(path):
        ...
        :param path the path of the file for which to fetch a photo object
    ...
    ############################################################################
    @staticmethod
    def create(file_basename, path, file_hash, created_struct):
        """
        Creates a new photo record in the database.

        :param file_basename: base name of the file (just the file name)
        :param path: path to the directory containing the file
        :param file_hash: SHA1 of the file
        :param created_struct: time struct object for when the photo was created

        :returns Photo:
        """
        inserted_id = Photo._create_work(file_basename, path, file_hash, created_struct)
        Logger.info("insert results: %s" % str(inserted_id))
        return Photo.get_by_id(inserted_id)
    
    @staticmethod
    def _create_work(file_basename, path, file_hash, created_struct):
        """
        Does the work of creating a new photo record in the database.
        ...
        :returns int: the id of the inserted photo
        """
        if created_struct is None:
    ...
    # INSTANCE METHODS
    ############################################################################
    def __init__(self, guard, photo_data, from_db=False):
        """
        Constructor; uses a guard to ensure noone else uses it.

        :param guard: Object which prevents Photo objects from being created in
                      a non-controlled manner (i.e. outside this class)
        :param photo_data: row from the database containing Photo information
        :param from_db: Whether this Photo is from the db or not
        """
        if guard != Photo.GUARD:
            raise Exception("Must only call __init__ from the accessor methods in Photo")
        self._id, self._filename, self._path, self._added, self._modified, self._image_date, self._hash, self._marked = photo_data
        self.new = from_db
        self._tags = None
        self._handle = None
    ...
    def get_dir_from_date(self):
        """
        Gets the target dir for this Photo from the created date
    ...
    def get_or_create_thumb(self, size=SMALL_THUMB_SIZE, path_only=False):
        ...
        :param size: which of the approved thumbnail sizes to get
        :param path_only: If true, do not create a thumbnail
    ...
    def move_file(self, src_dir=None, target_dir=None, copy=True, resolve=True):
        ...
        :param src_dir: Where to move the file from.
        :param target_dir: Where to move the file.
        :param copy: If True, copy the file rather than move
        :param resolve: if True, attempt to resolve file name conflicts.
        ...
        """
        if ((src_dir is None and target_dir is None)
            or (src_dir is not None and target_dir is not None)):
            raise Exception("Must define either src_dir or target_dir")

        src_dir = self.get_dir_from_date() if src_dir is None else src_dir
        src_path = os.path.join(src_dir, self.filename)
        target_dir = self.get_dir_from_date() if target_dir is None else target_dir
        ...
        if self.file_container is not None and self.file_container.file_handle is not None:
    ...
    def _values_from_photo_object(self, photo, set_dirty=True):
        ...
        :param photo: Photo to copy data from
        :param set_dirty: Whether to set the dirty flag upon copying data
        """
